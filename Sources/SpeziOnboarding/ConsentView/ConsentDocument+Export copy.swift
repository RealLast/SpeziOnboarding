...
extension ConsentDocument {
    ...
    private func split(markdown: AttributedString, pageSize: CGSize, headerHeight: CGFloat, footerHeight: CGFloat) -> (AttributedString, AttributedString) {
        ...
        while currentRange.location + currentRange.length < textStorage.length {
            // Expand currentRange by one character
            currentRange.length += 1

            // Calculate the used rect for the currentRange
            let usedRect = layoutManager.boundingRect(forGlyphRange: currentRange, in: textContainer)
            accumulatedHeight += usedRect.height

            // Check if adding one more character exceeds the contentHeight
            if accumulatedHeight > contentHeight {
                // If yes, adjust currentRange back by one character
                currentRange.length -= 1
                break
            }

            // Check if the current character is a line break or a paragraph break
            let currentChar = (markdown.string! as NSString).substring(with: currentRange)
            let currentCharIsNewline = currentChar == "\n"
            let currentCharIsParagraphBreak = currentChar == "\n\n"

            // If current character is a line break or paragraph break, break out of loop
            if currentCharIsNewline || currentCharIsParagraphBreak {
                break
            }
        }

        if currentRange.length > 0 {
            currentPage = AttributedString(textStorage.attributedSubstring(from: currentRange))
            let remainingRange = NSRange(location: currentRange.location + currentRange.length, length: textStorage.length - (currentRange.location + currentRange.length))
            remaining = AttributedString(textStorage.attributedSubstring(from: remainingRange))
        } else {
